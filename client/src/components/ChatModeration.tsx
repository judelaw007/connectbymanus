...
export default function ChatModeration() {
  ...
  const { data: flaggedMessages, isLoading: flaggedLoading } =
    trpc.chatModeration.getFlagged.useQuery(
      selectedChannel && view === "flagged"
        ? { channelId: selectedChannel.id }
        : undefined
    );

  const { data: channelMessages, isLoading: messagesLoading } =
    trpc.chatModeration.getChannelMessages.useQuery(
      selectedChannel && view === "messages"
        ? { channelId: selectedChannel.id, limit: 100 }
        : { channelId: 0, limit: 0 },
      { enabled: view === "messages" && !!selectedChannel }
    );
  ...
  // Total flagged count across all channels
  ...
  // Message list view (for a specific channel)
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <CardDescription className="mt-1 ml-9">
                  {messages.length} messages loaded. Flag messages for review or
          ...
          <AlertDialogContent>
            <AlertDialogHeader>
              ...
              <AlertDialogDescription>
                ...
                <strong>#{clearTargetChannel?.name}</strong>. This action cannot
                be undone.
              </AlertDialogDescription>
            </AlertDialogHeader>
            <AlertDialogFooter>
  ...
  // Flagged messages view (all channels or filtered)
  if (view === "flagged") {
    const flagged = flaggedMessages || [];
    // Build channel name lookup from channels data
    const channelMap = new Map(
      (channels || []).map((ch: any) => [ch.id, ch.name])
    );
        ...
        <Card>
          ...
          <CardContent>
            {flaggedLoading ? (
              ...
              <div className="py-12 text-center text-muted-foreground">
                No flagged messages. Messages flagged by admins will appear
                here.
              </div>
            ) : (
              <Table>
